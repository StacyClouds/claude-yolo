...
**Reason**: Superseded by `sandbox/host-claude-config`, which mounts the host's entire `~/.claude` folder (including `CLAUDE.md`) instead of just this one file.
**Migration**: No action needed - the host's `CLAUDE.md` remains available inside the sandbox, now as part of the full `~/.claude` mount.
...
**Reason**: Superseded by `sandbox/host-claude-config`'s equivalent requirement for the whole `~/.claude` folder.
**Migration**: No action needed - the replacement requirement covers the same guarantee for the full folder.
...
**Reason**: Superseded by `sandbox/host-claude-config`'s equivalent read-only requirement for the whole `~/.claude` folder.
**Migration**: No action needed - the replacement requirement covers the same guarantee for the full folder.
